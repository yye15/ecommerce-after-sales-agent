...
售后处理不是简单问答，而是一个多步骤决策流程。LangGraph 可以把任务拆成多个可解释节点，方便调试、替换和扩展。

### 为什么第一版 RAG 不用向量数据库？

当前政策库规模较小，覆盖的是质量、物流、客服、退款、价格权益和高风险投诉等固定场景。第一版使用本地 Markdown + 关键词检索，运行门槛低、过程透明、方便解释。后续知识库变大后，可以升级为 Chroma、FAISS 或混合检索。

### LALUN 在项目里做什么？

LALUN 是细粒度情感分析研究模块，用于后续增强 `aspect / opinion / sentiment` 三元组抽取能力。当前主流程默认使用 DeepSeek 和规则兜底，保证项目稳定可演示；LALUN 作为实验增强模块保留。
